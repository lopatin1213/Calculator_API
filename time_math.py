...
def _wrap_colon_blocks(expr: str) -> str:
    """
    Находит блоки вида 2h:30m или 1d:12h:30m и заменяет их на (2h+30m) или (1d+12h+30m).
    Это позволяет корректно обрабатывать умножение и деление целого временного блока.
    """
    # Ищем цепочки число+единица:число+единица[:...]
    ...
    while i < len(tokens):
        ...
        if tok.replace('.', '').isdigit():
            if i+1 < len(tokens) and tokens[i+1] in UNIT_TO_MS:
                ...
                new_tokens.append(tok)  # число без единицы
                i += 1
